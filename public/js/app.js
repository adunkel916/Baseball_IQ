...
const SCENARIOS = [
  ...
  },
  {
    ...
    options:[
      {text:"Stay at your position", correct:false, feedback:"The throw is going to 2nd base! Someone needs to be there to catch it!"},
  ...
  },
  {
    ...
    options:[
      ...
      {text:"Cover 2nd base", correct:false, feedback:"The runner already left 2nd. You need to get in the cutoff position to help relay the throw from the outfielder!"},
    ],
  },
  {
...
// ============ GAME STATE & AUDIO ============
// All paths are relative (no leading slash) to work with <base> tag
...
function startGame() {
  ...
  if (walkupSongs[typedName]) {
    ...
    currentWalkup.currentTime = 0;
    currentWalkup.play().catch(err => console.log("Audio failed:", err));
  }
